...
const buttons = [
  {
    cursor: -2,
    replaceWith: (selectedText) => `**${selectedText}**`,
    icon: (
      <svg
        width='11'
        height='13'
...
export default function RichText({ initialValue = '', setFieldValue }) {
  const textarea = useRef()
  const [value, setValue] = useState(initialValue)
  const [cursor, setCursor] = useState(null)

  const replaceSelection = (selectionStart, selectionEnd, text) => {
    return (
  ...
  const handleClick = (e, replaceWith, cursor) => {
    e.preventDefault()
    const selectionStart = textarea.current.selectionStart
    const selectionEnd = textarea.current.selectionEnd
    const selectedText = textarea.current.value.slice(
      selectionStart,
      selectionEnd
    )
    textarea.current.focus()
    setValue(
      replaceSelection(selectionStart, selectionEnd, replaceWith(selectedText))
    )
    setCursor(cursor)
  }
          ...
          {buttons.map((button, index) => {
            return (
              <li key={index}>
                <button
                  onClick={(e) =>
                    handleClick(e, button.replaceWith, button.cursor)
                  }
                >
                  {button.icon}
                </button>
